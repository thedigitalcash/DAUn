...
#include "ui_zdaucontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zdauTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zdauTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zDAU.";
            else if (!mint.isSeedCorrect)
                strReason = "The zDAU seed used to mint this zDAU is not the same as currently hold in the wallet";
            else
