...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered digitalaustraliandollar address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 DAU.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal digitalaustraliandollar:URI (digitalaustraliandollar:XyZ...?message=example)
            {
...
static void NotifyzDAUReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzDAUReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzDAUReset.connect(boost::bind(NotifyzDAUReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzDAUReset.disconnect(boost::bind(NotifyzDAUReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zdauTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
