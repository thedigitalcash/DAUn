...
CzDAUWallet::CzDAUWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdau seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDAUSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDAUSeed_deprecated()) {
                    LogPrintf("%s: Updated zDAU seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdau seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdau wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDAUWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDAUCount(nCountLastUsed);
    else if (!walletdb.ReadZDAUCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDAUWallet::Lock()
{
    seedMaster = 0;
}

void CzDAUWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDAUWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDAU(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDAUWallet::LoadMintPoolFromDB()
{
...
void CzDAUWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDAUWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDAUWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdauTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDAUWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDAU(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdauTracker which also adds to database
    pwalletMain->zdauTracker->Add(dMint, true);
...
void CzDAUWallet::SeedToZDAU(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDAUWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDAUWallet::GenerateDeterministicZDAU(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDAUWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDAUWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzDAUWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
