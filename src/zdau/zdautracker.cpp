...
CzDAUTracker::CzDAUTracker(std::string strWalletFile)
{
...
CzDAUTracker::~CzDAUTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDAUTracker::Init()
{
...
bool CzDAUTracker::Archive(CMintMeta& meta)
{
...
bool CzDAUTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDAUTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDAUTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDAUTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzDAUTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzDAUTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzDAUTracker::GetSerialHashes()
{
...
CAmount CzDAUTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDAUTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDAUTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDAUTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDAUTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDAUTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDAUTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDAUTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDAUTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDAUTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDAUTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzDAUWallet* zDAUWallet)
{
    bool iszDAUWalletInitialized = (NULL != zDAUWallet);
    CMintMeta meta;
    ...
    if (! iszDAUWalletInitialized)
        zDAUWallet = new CzDAUWallet(strWalletFile);
    meta.isSeedCorrect = zDAUWallet->CheckSeed(dMint);
    if (! iszDAUWalletInitialized)
        delete zDAUWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzDAUTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDAUTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDAUTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDAUTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDAUTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDAUTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzDAUWallet* zDAUWallet = new CzDAUWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zDAUWallet);
        }
        delete zDAUWallet;
        LogPrint("zero", "%s: added %d dzdau from DB\n", __func__, listDeterministicDB.size());
    }
