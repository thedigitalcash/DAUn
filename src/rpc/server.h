...
/**
 * DigitalAustralianDollar RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZdauSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzdauseed(const UniValue& params, bool fHelp);
extern UniValue getzdauseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzdau(const UniValue& params, bool fHelp);
extern UniValue dzdaustate(const UniValue& params, bool fHelp);
