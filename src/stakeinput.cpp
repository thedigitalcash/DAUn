...
// The zDAU block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zDAU is a hash of the serial
...
{
    //Create an output returning the zDAU that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZDAUOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zDAU output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zDAU", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZDAUOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zDAU output", __func__);
...
{
    CzDAUTracker* zdauTracker = pwallet->zdauTracker.get();
    CMintMeta meta;
    if (!zdauTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zdauTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a DAU stake is the outpoint
