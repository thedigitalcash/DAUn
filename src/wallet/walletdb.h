...
public:
    ...
    bool WriteZDAUSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDAUSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDAUSeed_deprecated(uint256& seed);
    bool EraseZDAUSeed();
    bool EraseZDAUSeed_deprecated();

    bool WriteZDAUCount(const uint32_t& nCount);
    bool ReadZDAUCount(uint32_t& nCount);
